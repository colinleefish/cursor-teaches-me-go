...
func displayCapacityOfGrowingSlice() {

	var mySlice = make([]int, 0, 10)
	fmt.Printf("Beginning capacity of growing slice: %d\n", cap(mySlice))

	var prevCapacity int = 10
	for i := 0; i < 1000000; i++ {
		mySlice = append(mySlice, i)
		if cap(mySlice) > prevCapacity {
			fmt.Printf("After %d appends: Length=%d, Capacity=%d, prevCapacity=%d, growth=%d\n", i+1, len(mySlice), cap(mySlice), prevCapacity, cap(mySlice)-prevCapacity)
			prevCapacity = cap(mySlice)
		}
	}
}
